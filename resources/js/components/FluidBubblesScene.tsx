    ...
    useFrame((state) => {
        ...
        if (
            ...
            if (!idleAnim) {
                ...
                const attractForce = new THREE.Vector3(
                    targetX,
                    targetY,
                    targetZ,
                )
                ...
                if (timeSinceIdle > 3.0) {
                    for (let i = 0; i < bubblePositions.current.length; i++) {
                        ...
                        if (dist < minDist && dist > 0.01) {
                            ...
                            const forceDir = currentPos.current
                                .clone()
                                .sub(otherPos)
                                .normalize();
                            ...
                            velocity.current.add(
                                forceDir.multiplyScalar(pushStrength),
                            );
                        }
                    }
                }
            ...
            const currentScale =
                scale * Math.min(1, Math.max(0.01, easeOutBack));
...
function Bubbles({ isDark }: { isDark: boolean }) {
    ...
    const bubblePositions = useRef(
        bubblesData.map(() => new THREE.Vector3(0, -25, 0)),
    );
    const scales = useMemo(
        () => bubblesData.map((d) => d.scale),
        [bubblesData],
    );
    ...
    return (
        ...
        >
            <Canvas camera={{ position: [0, 0, 18], fov: 45 }}>
                ...
                <directionalLight
                    position={[10, 10, 5]}
                    intensity={activeIsDark ? 2 : 2.5}
                    color={activeIsDark ? '#aa44ff' : '#ffffff'}
                />
                <directionalLight
                    position={[-10, -10, -5]}
                    intensity={activeIsDark ? 1 : 1.5}
                    color={activeIsDark ? '#006aff' : '#ffeedd'}
                />
                <pointLight
                    position={[0, 0, 5]}
                    intensity={activeIsDark ? 1 : 0.5}
                    color={activeIsDark ? '#ff0055' : '#ffffff'}
                />
